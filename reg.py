...
# ─── بيانات الموظفات المدمجة (115 موظفة من ملفات Excel) ──────────
WHITELIST_DATA = [
...
# ══════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.admin_logged_in:
        ...
        # ── تهيئة القائمة البيضاء ───────────────────────────────────
            ...
            st.markdown('<div class="admin-section">رفع بيانات 115 موظفة من ملفات Excel مباشرة لـ Google Sheets</div>', unsafe_allow_html=True)

            wl_now   = get_whitelist()
            total    = len(WHITELIST_DATA)
            existing = len(wl_now)
            new_ones = sum(1 for r in WHITELIST_DATA if str(r[0]) not in wl_now)

            c1, c2, c3 = st.columns(3)
            c1.metric("إجمالي الموظفات", total)
            c2.metric("موجودات مسبقاً", existing)
            c3.metric("جديدات للرفع", new_ones)

            if new_ones == 0:
                st.success("✅ جميع الموظفات موجودات في القائمة البيضاء — لا يوجد جديد للرفع")
            else:
                st.info(f"سيتم رفع {new_ones} موظفة جديدة لـ Google Sheets")

            col_a, col_b = st.columns(2)

            with col_a:
                if st.button(f"🚀 رفع {new_ones} موظفة جديدة", use_container_width=True, key="btn_init_new"):
                    progress = st.progress(0, text="جاري الرفع...")
                    uploaded = 0
                    errors   = 0
                    wl_now   = get_whitelist()
                    to_upload = [r for r in WHITELIST_DATA if str(r[0]) not in wl_now]
                    for i, r in enumerate(to_upload):
                        try:
                            whitelist_sheet.append_row([r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7]])
                            uploaded += 1
                        except Exception:
                            errors += 1
                        progress.progress((i+1)/len(to_upload), text=f"جاري الرفع... {i+1}/{len(to_upload)}")
                    log_audit("أدمن","النظام","تهيئة القائمة البيضاء",
                              f"رُفع {uploaded} موظفة | أخطاء: {errors}")
                    if errors == 0:
                        st.success(f"✅ تم رفع {uploaded} موظفة بنجاح!")
                    else:
                        st.warning(f"⚠️ تم رفع {uploaded} | فشل {errors}")
                    st.rerun()

            with col_b:
                if st.button("🔄 إعادة رفع الكل (تحديث)", use_container_width=True, key="btn_init_all"):
                    if st.session_state.get("confirm_reupload"):
                        progress = st.progress(0, text="جاري الرفع الكامل...")
                        try:
                            whitelist_sheet.clear()
                            whitelist_sheet.append_row(["الرقم الشخصي","الاسم","المدرسة","المهمة","رقم التواصل","البريد الإلكتروني","المسمى الوظيفي","نشط"])
                        except Exception as e:
                            st.error(f"خطأ في مسح الورقة: {e}")
                        uploaded = 0
                        for i, r in enumerate(WHITELIST_DATA):
                            try:
                                whitelist_sheet.append_row([r[0],r[1],r[2],r[3],r[4],r[5],r[6],r[7]])
                                uploaded += 1
                            except Exception:
                                pass
                            progress.progress((i+1)/len(WHITELIST_DATA), text=f"{i+1}/{len(WHITELIST_DATA)}")
                        log_audit("أدمن","النظام","إعادة رفع كامل للقائمة البيضاء",f"رُفع {uploaded} موظفة")
                        st.success(f"✅ تم إعادة رفع {uploaded} موظفة بنجاح!")
                        st.session_state.confirm_reupload = False
                        st.rerun()
                    else:
                        st.session_state.confirm_reupload = True
                        st.warning("⚠️ هذا سيمسح القائمة الحالية ويعيد رفع الكل — اضغطي مرة ثانية للتأكيد")

            # معاينة البيانات
            st.markdown('<div class="admin-section">معاينة بيانات الموظفات</div>', unsafe_allow_html=True)
            for r in WHITELIST_DATA[:5]:
                task_short = r[3].split('/')[0].strip()
                st.markdown(
                    f'<div class="audit-row"><span class="ar-op">{r[1]}</span>' +
                    f'<div class="ar-det">#{r[0]} — {r[2]} — {task_short}</div></div>',
                    unsafe_allow_html=True
                )
            st.markdown(f'<div class="audit-row"><span class="ar-op" style="color:#888;">... و {len(WHITELIST_DATA)-5} موظفة أخرى</span></div>', unsafe_allow_html=True)
